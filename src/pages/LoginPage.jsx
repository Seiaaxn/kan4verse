...
import { getUser, getStoredUsers, ensureAdminExists } from '../utils/userSystem';
...
const LoginPage = () => {
    ...
    useEffect(() => { if (getUser()) navigate('/', { replace: true }); }, []);
    ...
    const handleLogin = async (e) => {
        ...
        const session = { id: found.id, username: found.username, email: found.email, joinedAt: found.joinedAt, role: found.role || 'user', xp: found.xp || 0, avatar: found.avatar || null };
    ...
    const handleRegister = async (e) => {
        ...
        const newUser = { id: Date.now().toString(), username: form.username, email: form.email, password: form.password, joinedAt: new Date().toISOString(), role: isFirst ? 'admin' : 'user', xp: 0, avatar: null };
        localStorage.setItem(USERS_KEY, JSON.stringify([...users, newUser]));
        const session = { id: newUser.id, username: newUser.username, email: newUser.email, joinedAt: newUser.joinedAt, role: newUser.role, xp: 0, avatar: null };
        localStorage.setItem(AUTH_KEY, JSON.stringify(session));
        setLoading(false);
        navigate('/', { replace: true });
    };
        ...
        <div className="min-h-screen bg-dark-bg flex flex-col relative overflow-hidden">
            ...
            <div className="relative z-10 flex-1 px-5 py-2 flex flex-col max-w-sm mx-auto w-full">
                ...
                <div className="text-center mb-8">
                    <div className="inline-flex items-center justify-center w-16 h-16 rounded-3xl mb-4 shadow-2xl animate-float"
                ...
                {/* Tabs */}
                <div className="flex p-1 mb-6 rounded-2xl" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                ...
                {/* Error */}
                {error && (
                    <div className="flex items-center gap-2.5 px-4 py-3 mb-4 rounded-2xl animate-fade-in"
                        style={{ background: 'rgba(250,109,154,0.1)', border: '1px solid rgba(250,109,154,0.2)' }}>
                ...
                <form onSubmit={tab === 'login' ? handleLogin : handleRegister} className="space-y-3">
                    {tab === 'register' && (
                        <div className="relative">
                            ...
                            <input value={form.username} onChange={e => set('username', e.target.value)}
                                ...
                                className="w-full pl-11 pr-4 py-4 rounded-2xl text-sm text-white outline-none transition-all"
                    ...
                    <div className="relative">
                        ...
                        <input type="email" value={form.email} onChange={e => set('email', e.target.value)}
                            placeholder="Email"
                            className="w-full pl-11 pr-4 py-4 rounded-2xl text-sm text-white outline-none"
                            style={{ background: 'var(--card)', border: '1px solid var(--border)' }} />
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => set('password', e.target.value)}
                            ...
                            className="w-full pl-11 pr-12 py-4 rounded-2xl text-sm text-white outline-none"
                            style={{ background: 'var(--card)', border: '1px solid var(--border)' }} />
                ...
                {tab === 'register' && (
                    <p className="text-xs text-center mt-4" style={{ color: 'var(--muted)' }}>
                        🎯 Pengguna pertama otomatis jadi <span style={{ color: '#fa6d9a', fontWeight: 700 }}>Admin</span>
                    </p>
                )}

                <button onClick={() => setTab(tab === 'login' ? 'register' : 'login')}
                    className="text-xs text-center mt-5 w-full" style={{ color: 'var(--muted)' }}>
                    {tab === 'login' ? 'Belum punya akun? ' : 'Sudah punya akun? '}
                    <span style={{ color: '#7c6dfa', fontWeight: 700 }}>
